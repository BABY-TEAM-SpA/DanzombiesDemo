...
public class SequenceStep
{
    ...
    [SerializeField] public List<DanceStep> pattern = new();
    ...
    #region [METHODS]
    public void ApplyDance(bool isCorrect) => playerCorrectDancesOnSequence = isCorrect
        ? playerCorrectDancesOnSequence+1 : 0;
        ...
        if (pattern.Count != 0 && puzzleCounter > 0)
        {
            switch (playbackMode)
            {
                ...
                case PlaybackMode.LoopUntilXCorrectSteps:
                    ...
                    {
                        nextStep = CalulateFutureStep(nextStepCounter);
                        if (nextStepCounter%pattern.Count ==0)
                            playerCorrectDancesOnSequence = 0;
                        return null;
                    }
            }
        }
